use crate::mapper;
use rustling::{Candidate, MaxElementTagger, ParsedNode, ParserMatch, Range, Value};
use rustling_ontology_values::{Dimension, OutputKind, ParsingContext};
    ...
    fn tag(
        ...
        let mut candidates = candidates
            ...
            .filter_map(|(parsed_node, parser_match)| {
                if parsed_node.value.is_too_ambiguous() {
                    None
                } else {
        ...
        candidates.sort_by(|a, b| {
            a.1.byte_range
                .len()
                .cmp(&b.1.byte_range.len())
                .then_with(|| a.1.byte_range.0.cmp(&b.1.byte_range.0))
                ...
                .then_with(|| b.1.parsing_tree_height.cmp(&a.1.parsing_tree_height))
                .then_with(|| b.1.parsing_tree_num_nodes.cmp(&a.1.parsing_tree_num_nodes))
        });
        ...
        candidates
            ...
            .map(|c| {
                if selected_ranges
                    .iter()
                    .all(|a| a.is_disjoint(&c.1.byte_range))
                {
