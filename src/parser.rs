...
pub fn extract_node_features(
    node: &rustling::Node<Payload>,
) -> rustling::Input<rustling::RuleId, Feat> {
    let grains_feat = node
        .children
        .iter()
        .filter_map(|c| c.payload.map(|p| p.0 as usize))
        .collect::<Vec<_>>();
    let rules_feat = node
        .children
        .iter()
    ...
    if !grains_feat.is_empty() {
        features.push(Feat::Grains(grains_feat));
    }
...
#[cfg(test)]
mod tests {
    use crate::{build_raw_parser, CandidateTagger, Lang, OutputKind};
    ...
    fn test_twenty() {
        ...
        let result = parser.parse("twenty", &tagger).unwrap();
    ...
    fn test_21() {
        ...
        let result = parser.parse("twenty-one", &tagger).unwrap();
        assert_eq!(
            21,
