use regex::Regex;
use rustling::{RuleError, RuleResult, RuleSetBuilder, RustlingResult};
...
pub fn rules_finance(b: &mut RuleSetBuilder<Dimension>) -> RustlingResult<()> {
    ...
    b.rule_1_terminal("KRW", b.reg(r#"₩|원|krw"#)?, |_| {
        Ok(MoneyUnitValue { unit: Some("KRW") })
    });
    b.rule_1_terminal("$", b.reg(r#"\$|달러|불"#)?, |_| {
        Ok(MoneyUnitValue { unit: Some("$") })
    });
    b.rule_1_terminal("cent", b.reg(r#"cents?|센[트|츠]|c|¢"#)?, |_| {
        Ok(MoneyUnitValue { unit: Some("cent") })
    });
    b.rule_1_terminal("EUR", b.reg(r#"€|유로|euro?"#)?, |_| {
        Ok(MoneyUnitValue { unit: Some("EUR") })
    });
    b.rule_1_terminal("£", b.reg(r#"£|파운드|영국파운드"#)?, |_| {
        Ok(MoneyUnitValue { unit: Some("£") })
    });
    b.rule_1_terminal("GBP", b.reg(r#"gbp"#)?, |_| {
        Ok(MoneyUnitValue { unit: Some("GBP") })
    });
    b.rule_1_terminal("AUD", b.reg(r#"aud|호주달러"#)?, |_| {
        Ok(MoneyUnitValue { unit: Some("AUD") })
    });
    b.rule_1_terminal("USD", b.reg(r#"us[d\$]"#)?, |_| {
        Ok(MoneyUnitValue { unit: Some("USD") })
    });
    b.rule_1_terminal("PTS", b.reg(r#"pta?s?"#)?, |_| {
        Ok(MoneyUnitValue { unit: Some("PTS") })
    });
    b.rule_1_terminal(
        "INR",
...
pub fn rules_datetime(b: &mut RuleSetBuilder<Dimension>) -> RustlingResult<()> {
    ...
    b.rule_1_terminal("New Year's Day", b.reg(r#"신정|새해 첫 날"#)?, |_| {
    ...
    b.rule_1_terminal("Independence Movement Day", b.reg(r#"삼일절"#)?, |_| {
        helpers::month_day(3, 1)
    });
    b.rule_1_terminal("Children's Day", b.reg(r#"어린이날"#)?, |_| {
        helpers::month_day(5, 5)
    });
    b.rule_1_terminal("Memorial Day", b.reg(r#"현충일"#)?, |_| {
        helpers::month_day(6, 6)
    });
    b.rule_1_terminal("Constitution Day", b.reg(r#"제헌절"#)?, |_| {
        helpers::month_day(6, 17)
    });
    b.rule_1_terminal("Liberation Day", b.reg(r#"광복절"#)?, |_| {
        helpers::month_day(8, 15)
    });
    b.rule_1_terminal("National Foundation Day", b.reg(r#"개천절"#)?, |_| {
        helpers::month_day(10, 3)
    });
    b.rule_1_terminal("Hangul Day", b.reg(r#"한글날"#)?, |_| {
        helpers::month_day(10, 9)
    });
    b.rule_1_terminal(
        "christmas eve",
        b.reg(r#"(크리스마스)?이브"#)?,
    ...
    b.rule_1_terminal("christmas", b.reg(r#"크리스마스|성탄절"#)?, |_| {
    ...
    b.rule_1_terminal("Father's day", b.reg(r#"아버지\s?날"#)?, |_| {
        let sundays_of_june = helpers::month(6)?.intersect(&helpers::day_of_week(Weekday::Sun)?)?;
        let second_week_of_june =
            helpers::cycle_nth_after(Grain::Week, 2, &helpers::month_day(6, 1)?)?;
        sundays_of_june.intersect(&second_week_of_june) // third sunday of June
    });
    b.rule_1_terminal("Mother's day", b.reg(r#"어머니\s?날"#)?, |_| {
        let sundays_of_may = helpers::month(5)?.intersect(&helpers::day_of_week(Weekday::Sun)?)?;
        let first_week_of_may =
            helpers::cycle_nth_after(Grain::Week, 1, &helpers::month_day(5, 1)?)?;
        sundays_of_may.intersect(&first_week_of_may) // second sunday of May
    });
    b.rule_1_terminal("Parents day", b.reg(r"어버이\s?날")?, |_| {
        helpers::month_day(5, 8)
    });
    b.rule_1_terminal("Teachers' day", b.reg(r#"스승의\s?날"#)?, |_| {
        helpers::month_day(5, 15)
    });
    b.rule_1_terminal(
        "Labor Day",
    ...
    b.rule_1_terminal("White Day", b.reg(r#"화이트\s?데이"#)?, |_| {
        helpers::month_day(3, 14)
    });
    b.rule_1_terminal("Coming-of-Age Day", b.reg(r#"성년의\s?날"#)?, |_| {
        let mondays_of_may = helpers::month(5)?.intersect(&helpers::day_of_week(Weekday::Mon)?)?;
        let third_week_of_may =
            helpers::cycle_nth_after(Grain::Week, 3, &helpers::month_day(5, 1)?)?;
        mondays_of_may.intersect(&third_week_of_may) // third monday of May
    });
    b.rule_1_terminal("First Dog Days", b.reg(r#"초복"#)?, |_| {
        helpers::month_day(7, 13)
    });
    b.rule_1_terminal("Second Dog Days", b.reg(r#"중복"#)?, |_| {
        helpers::month_day(7, 23)
    });
    b.rule_1_terminal("Last Dog Days", b.reg(r#"말복"#)?, |_| {
        helpers::month_day(8, 12)
    });
    b.rule_1_terminal(
        "Halloween",
    ...
    b.rule_1_terminal("Armed Forces Day", b.reg(r#"국군의\s?날"#)?, |_| {
        helpers::month_day(10, 1)
    });
    b.rule_1_terminal("Couple’s Day", b.reg(r#"부부의\s?날"#)?, |_| {
        helpers::month_day(5, 21)
    });
    b.rule_1_terminal("Elderly Day", b.reg(r#"노인의\s?날"#)?, |_| {
        helpers::month_day(10, 2)
    });
    b.rule_1_terminal("Dokdo Day", b.reg(r#"독도의\s?날"#)?, |_| {
        helpers::month_day(10, 25)
    });
    b.rule_1_terminal("now", b.reg(r#"방금|지금|방금|막|이제"#)?, |_| {
        helpers::cycle_nth(Grain::Second, 0)
    });
    b.rule_1_terminal("today", b.reg(r#"오늘|당일|금일"#)?, |_| {
        helpers::cycle_nth(Grain::Day, 0)
    });
    b.rule_1_terminal("tomorrow", b.reg(r#"내일|명일|낼"#)?, |_| {
        helpers::cycle_nth(Grain::Day, 1)
    });
    b.rule_1_terminal("yesterday", b.reg(r#"어제|작일|어저께"#)?, |_| {
        helpers::cycle_nth(Grain::Day, -1)
    });
    b.rule_1_terminal(
        "in two years",
    ...
    b.rule_1_terminal("two years ago", b.reg(r#"재작년"#)?, |_| {
        helpers::cycle_nth(Grain::Year, -2)
    });
    b.rule_1_terminal("three years ago", b.reg(r#"재재작년"#)?, |_| {
        helpers::cycle_nth(Grain::Year, -3)
    });
    b.rule_2(
        "start of week",
        ...
        |week, _| {
            ...
            let end = week
                .value()
                .intersect(&helpers::day_of_week(Weekday::Tue)?)?;
    ...
    b.rule_2(
        ...
        |week, _| {
            let start = week
                .value()
                .intersect(&helpers::day_of_week(Weekday::Fri)?)?;
            let end = week
                .value()
    ...
    b.rule_3(
        "<datetime> nth <datetime> - 3월 첫째 화요일",
        datetime_check!(),
        ordinal_check!(),
        datetime_check!(),
        |a, ordinal, b| {
    ...
    b.rule_3(
        "<datetime> nth <cycle> - 3월 첫째 화요일",
        datetime_check!(),
        ordinal_check!(),
        cycle_check!(),
        |datetime, ordinal, cycle| {
            helpers::cycle_nth_after_not_immediate(
                cycle.value().grain,
    ...
    b.rule_4(
        "<datetime> nth of <cycle> - 3월 첫째 화요일",
        datetime_check!(),
        b.reg(r#"의"#)?,
        ordinal_check!(),
        cycle_check!(),
        |datetime, _, ordinal, cycle| {
            helpers::cycle_nth_after_not_immediate(
                cycle.value().grain,
                ordinal.value().value - 1,
                datetime.value(),
    ...
    b.rule_1_terminal("noon", b.reg(r#"정오|오정|한낮"#)?, |_| {
    ...
    b.rule_1_terminal("morning", b.reg(r#"아침"#)?, |_| {
        Ok(helpers::hour(4, false)?
            .span_to(&helpers::hour(12, false)?, false)?
            .form(Form::PartOfDay(PartOfDayForm::Morning)))
    });
    b.rule_1_terminal("morning (latent)", b.reg(r#"오전"#)?, |_| {
        Ok(helpers::hour(4, false)?
            .span_to(&helpers::hour(12, false)?, false)?
            .latent()
            .form(Form::PartOfDay(PartOfDayForm::Morning)))
    ...
    b.rule_1_terminal("afternoon", b.reg(r#"오후"#)?, |_| {
        Ok(helpers::hour(12, false)?
            .span_to(&helpers::hour(19, false)?, false)?
            .form(Form::PartOfDay(PartOfDayForm::Afternoon)))
    });
    b.rule_1_terminal(
        "late afternoon (latent)",
        b.reg(r#"늦은 오후|오후 늦게"#)?,
        |_| {
    ...
    b.rule_1_terminal("evening", b.reg(r#"저녁"#)?, |_| {
        Ok(helpers::hour(18, false)?
    ...
    b.rule_1_terminal("night", b.reg(r#"밤"#)?, |_| {
    ...
    b.rule_1_terminal(
        ...
        b.reg(r#"저녁(?: ?(?:식사|밥))?"#)?,
        |_| {
            Ok(helpers::hour_minute(17, 30, false)?
                .span_to(&helpers::hour(21, false)?, false)?
                .latent()
                .form(Form::Meal))
        },
    ...
    b.rule_2(
        ...
        datetime_check!(
            |datetime: &DatetimeValue| form!(Form::PartOfDay(_))(datetime)
                || form!(Form::Meal)(datetime)
        ),
        b.reg(r#"에|동안|때"#)?,
        |datetime, _| Ok(datetime.value().clone().not_latent()),
    );
    ...
    b.rule_1_terminal("week-end", b.reg(r#"주말"#)?, |_| {
        let friday = helpers::day_of_week(Weekday::Fri)?.intersect(&helpers::hour(18, false)?)?;
    ...
    b.rule_3(
        ...
        datetime_check!(
            |datetime: &DatetimeValue| if let Form::TimeOfDay(_) = datetime.form {
                !datetime.latent
            } else {
                false
            }
    ...
    b.rule_2(
        ...
        b.reg(r#"이내에?"#)?,
        |duration, _| {
            helpers::cycle_nth(Grain::Second, 0)?.span_to(&duration.value().in_present()?, false)
        },
    );
    ...
    b.rule_4(
        ...
        duration_check!(),
        b.reg(r#"후에|뒤에"#)?,
        |datetime, _, duration, _| duration.value().after(datetime.value()),
    );
    Ok(())
}

pub fn rules_duration(b: &mut RuleSetBuilder<Dimension>) -> RustlingResult<()> {
    b.rule_1_terminal("second (unit-of-duration)", b.reg(r#"초"#)?, |_| {
        Ok(UnitOfDurationValue::new(Grain::Second))
    });
    b.rule_1_terminal("minute (unit-of-duration)", b.reg(r#"분"#)?, |_| {
        Ok(UnitOfDurationValue::new(Grain::Minute))
    });
    b.rule_1_terminal("hour (unit-of-duration)", b.reg(r#"시간?"#)?, |_| {
        Ok(UnitOfDurationValue::new(Grain::Hour))
    });
    b.rule_1_terminal("day (unit-of-duration)", b.reg(r#"날|일간?"#)?, |_| {
        Ok(UnitOfDurationValue::new(Grain::Day))
    });
    b.rule_1_terminal(
    ...
    // TODO check that a cycle is necessary for this rule and not a unit of duration (hour)
    b.rule_2(
        "half an hour",
    ...
    b.rule_1_terminal("a day - 하루", b.reg(r#"하루"#)?, |_| {
        Ok(DurationValue::new(PeriodComp::days(1).into()))
    });
    b.rule_2(
        ...
        |integer, uod| {
            Ok(DurationValue::new(
                PeriodComp::new(uod.value().grain, integer.value().value).into(),
            ))
        },
    );
    b.rule_2_terminal(
        ...
        |text_match, _| {
            ...
            Ok(DurationValue::new(
                PeriodComp::new(Grain::Minute, decimal_hour).into(),
            ))
        },
    );
    b.rule_2(
        ...
        |integer, _| {
            Ok(DurationValue::new(
                PeriodComp::new(Grain::Minute, integer.value().value * 60 + 30).into(),
            ))
        },
    );
    b.rule_2(
    ...
    b.rule_2(
        ...
        duration_check!(),
        b.reg(r#"(?:이 ?)후|부터"#)?,
        |duration, _| Ok(duration.value().in_present()?.mark_after_start()),
    );
    b.rule_3(
        "<duration> from now",
        b.reg(r#"지금부터|현시간부터"#)?,
    ...
    b.rule_1_terminal(
        ...
        b.reg(
            r#"(하루|이틀|양일|(?:사|나)흘|(?:닷|엿)새|(?:이|여드|아흐)레|열흘|열하루)"#,
        )?,
        |text_match| {
            ...
            Ok(DurationValue::new(
                PeriodComp::new(Grain::Day, number_of_days).into(),
            ))
        },
    );
    Ok(())
...
pub fn rules_cycle(b: &mut RuleSetBuilder<Dimension>) -> RustlingResult<()> {
    b.rule_1_terminal("second (cycle)", b.reg(r#"초"#)?, |_| {
        CycleValue::new(Grain::Second)
    });
    b.rule_1_terminal("minute (cycle)", b.reg(r#"분"#)?, |_| {
        CycleValue::new(Grain::Minute)
    });
    b.rule_1_terminal("hour (cycle)", b.reg(r#"시간?"#)?, |_| {
        CycleValue::new(Grain::Hour)
    });
    b.rule_1_terminal("day (cycle)", b.reg(r#"날|일간?"#)?, |_| {
        CycleValue::new(Grain::Day)
    });
    b.rule_1_terminal("week (cycle)", b.reg(r#"주(?:간|일)?"#)?, |_| {
        CycleValue::new(Grain::Week)
    });
    b.rule_1_terminal("month (cycle)", b.reg(r#"(?:달|개?월)"#)?, |_| {
        CycleValue::new(Grain::Month)
    });
    b.rule_1_terminal("quarter (cycle)", b.reg(r#"분기"#)?, |_| {
        CycleValue::new(Grain::Quarter)
    });
    b.rule_1_terminal("year (cycle)", b.reg(r#"해|(?:연|년)간?"#)?, |_| {
        CycleValue::new(Grain::Year)
    });
    b.rule_2(
        "this <cycle>",
        b.reg(r#"이번?|금|올|돌아오는"#)?,
    ...
    b.rule_3(
        ...
        cycle_check!(),
        |_, integer, cycle| {
            helpers::cycle_n_not_immediate(cycle.value().grain, -1 * integer.value().value)
        },
    );
    b.rule_3(
        "next n <cycle>",
    ...
    b.rule_3(
        ...
        cycle_check!(|cycle: &CycleValue| cycle.grain == Grain::Quarter),
        |datetime, integer, _| {
            helpers::cycle_nth_after(Grain::Quarter, integer.value().value - 1, datetime.value())
        },
    );
    Ok(())
}
...
pub fn rules_numbers(b: &mut RuleSetBuilder<Dimension>) -> RustlingResult<()> {
    ...
    b.rule_1_terminal("integer (numeric)", b.reg(r#"(\d{1,18})"#)?, |text_match| {
    ...
    b.rule_1_terminal("integer 0", b.reg(r#"영|공|빵"#)?, |_| {
        IntegerValue::new(0)
    });
    b.rule_1_terminal("half - 반", b.reg(r#"반"#)?, |_| FloatValue::new(0.5));
    b.rule_1_terminal("few 몇", b.reg(r#"몇"#)?, |_| {
        Ok(IntegerValue {
            value: 3,
            precision: Precision::Approximate,
    ...
    b.rule_1_terminal(
        ...
        |text_match| {
            ...
            let coef_1000000000000 = groups
                .get(1)
                .and_then(|g| *g)
                .map(|g| get_number(g))
                .unwrap_or(Ok((0, None)))?;
            let coef_100000000 = groups
                .get(2)
                .and_then(|g| *g)
                .map(|g| get_number(g))
                .unwrap_or(Ok((0, None)))?;
            let coef_10000 = groups
                .get(3)
                .and_then(|g| *g)
                .map(|g| {
                    if g == "만" {
            ...
            let coef_1 = groups
                ...
                .map(|g| get_number(g))
                .unwrap_or(Ok((0, None)))?;
            let number = 1000000000000 * coef_1000000000000.0
                + 100000000 * coef_100000000.0
                + 10000 * coef_10000.0
                + coef_1.0;
    ...
    b.rule_1_terminal("decimal number", b.reg(r#"(\d*\.\d+)"#)?, |text_match| {
        FloatValue::new(text_match.group(1).parse()?)
    });
