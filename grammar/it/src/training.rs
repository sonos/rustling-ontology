...
pub fn examples_time(v: &mut Vec<::rustling::train::Example<Dimension>>) {
    ...
    example!(v, check_moment!(c, [2013, 3, 2, 5]),
    // "il 2 alle 5h",
    "il 2 alle 5 del mattino"
    );
    example!(v, check_moment!(c, [2013, 2, 16, 18]),
    "il 16 alle 18",
    // "il 16 verso le 18:00", "il 16 tipo verso le 18",
    "il 16 alle 6 del pomeriggio",
    "il 16 alle 6 di pomeriggio",
    // "il 16 intorno alle 18h", "il 16 verso le 6 di sera",
    "sabato 16 alle 6 della sera"
    );
    ...
    example!(v, check_moment!(c, [2013, 2, 15, 12]),
    ...
    example!(v, check_moment!(c, [2013, 2, 12, 4, 30, 1]),
    "tra un secondo",
    "fra un secondo",
    // "a un secondo da ora", "a un secondo da adesso",
    "tra un sec");
    example!(v, check_moment!(c, [2013, 2, 12, 4, 31, 0]),
    "tra un minuto",
    "fra un minuto"//,
    // "a un minuto da ora", "a un minuto da adesso", "tra un min"
    );
    ...
    example!(v, check_moment_span!(c, [2013, 2, 12, 15], [2013, 2, 12, 17]),
             //"a metà pomeriggio", "a metà del pomeriggio",
    "nel mezzo del pomeriggio"
    //, "alla metà del pomeriggio"
    );
    example!(v, check_moment_span!(c, [2013, 2, 12, 4], [2013, 2, 12, 9]),
    //"a inizio mattinata",
    "la mattina presto",
    "la mattina sul presto",
    "di prima mattina",
    "di primo mattino",
    "nel mattinata sul presto"//,
    ...
    example!(v, check_moment_span!(c, [2013, 2, 12, 10], [2013, 2, 12, 12]), //"a fine mattina", "a fine mattinata", "alla fine della mattina", "alla fine della mattinata",
    "nella tarda mattinata",
    //"la mattina sul tardi",
    "la seconda mattinata");
    example!(v, check_moment_span!(c, [2013, 2, 12, 13], [2013, 2, 12, 17]),
    "dopo pranzo"//,
    //"dopo ora di pranzo"
    );
    example!(v, check_moment_span!(c, [2013, 2, 12, 10], [2013, 2, 12, 12]), "prima di pranzo");
    example!(v, check_moment_span!(c, [2013, 2, 12, 12], [2013, 2, 12, 14]),
    "durante il pranzo"//,
    //"all'ora di pranzo"
    );
    example!(v, check_moment_span!(c, [2013, 2, 12, 17], [2013, 2, 12, 21]), "dopo il lavoro", "dopo l'orario di lavoro");
    example!(v, check_moment_span!(c, [2013, 2, 12, 12], [2013, 2, 12, 15]),
    "nel primo pomeriggio",
    //"all'inizio del pomeriggio",
    "il pomeriggio presto",
    "il pomeriggio sul presto"//,
    //"a inizio pomeriggio"
    );
    ...
    example!(v, check_moment_span!(c, [2013, 2, 12, 11], [2013, 2, 12, 16]),
    //"a metà giornata",
    ...
    // Explicit intervals
    example!(v, check_moment_span!(c, [2013, 7, 13], [2013, 7, 16]),
    "13-15 luglio",
    "dal 13 al 15 luglio",
    "dal 13 fino al 15 luglio",
    "dal 13 luglio al 15 luglio",
    //"13 luglio - 15 luglio",
    "tra il 13 e il 15 luglio",
    "da sabato 13 a domenica 15 luglio"//,
    //"dal sabato 13 alla domenica 15 di luglio",
    //"dal 13 alla domenica 15 luglio"
    );
    example!(v, check_moment_span!(c, [2013, 7, 1], [2013, 7, 11]),
    "dall'1 al 10 luglio",
    "da lunedì 1 a mercoledì 10 luglio",
    "dal lunedì 1 al mercoledì 10 luglio",
    // FIXME: Resolution issue
    //"dal lunedì primo al mercoledì 10 di luglio",
    "dal 1° luglio al mercoledì 10 luglio"
    );
    example!(v, check_moment_span!(c, [2013, 2, 13], [2013, 2, 19]),
    // FIXME: Recognized but as time-of-day interval
    //"dal 13 al 18",
    "tra il 13 e il 18"//,
    //"fra il 13 e il 18"
    );
    example!(v, check_moment_span!(c, [2023, 2, 1], [2023, 4, 1]),
    "tra febbraio e marzo due mila venti tre",
    "tra il febbraio e il marzo del due mila venti tre"
    );
    example!(v, check_moment_span!(c, [2013, 6, 10], [2013, 7, 2]),
    "dal 10 giugno al primo luglio",
    "tra il 10 di giugno e l'uno di luglio",
    //"fra il 10 di giugno e il primo luglio",
    "dal 10 giugno al 1° luglio"
    );
    example!(v, check_moment_span!(c, [2017,4,6], [2017,6,9]),
    "dal sei aprile all'otto giugno due mila diciassette",
    "dal sei di aprile all'otto di giugno del due mila diciassette"
    );
    ...
    example!(v, check_moment_with_direction!(c, [2013, 3, 8], Direction::After),
    "a partire dall'8"//,
    //"dall'8 marzo",
    //"dall'8 di marzo in poi"
    );
    ...
    example!(v, check_moment_with_direction!(c, [2013, 2, 15, 12], Direction::After),
    "venerdì a partire da mezzogiorno"//,
    //"venerdì da mezzogiorno in poi",
    //"dal mezzodì di venerdi"
    );
    ...
    example!(v, check_moment_span!(c, [2013, 2, 12, 4, 30, 0], [2013, 2, 26]),
    "entro 2 settimane"//,
    //"in due settimane"
    );
    // 15j != 2 settimanas
    example!(v, check_moment_span!(c, [2013, 2, 12, 4, 30, 0], [2013, 5, 12]),
    "entro 3 mesi"//,
    // "in tre mesi"
    );
    example!(v, check_moment_span!(c, [2013, 2, 12, 4, 30, 0], [2013, 2, 27]),
    //"in 15 giorni",
    ...
    example!(v, check_moment_span!(c, [2013, 3, 25], [2013, 4, 1]),
    "fine marzo"//,
    //"alla fine del mese di marzo",
    //"a fine marzo",
    //"alla fine di marzo"
    );
    example!(v, check_moment_span!(c, [2013, 4, 1], [2013, 4, 6]),
    "inizio aprile"//,
    //"all'inizio del mese d'aprile",
    ...
    example!(v, check_moment!(c, [2013, 3]),
    "marzo",
    "a marzo",
    "in marzo",
    //"nel mese di marzo",
...
pub fn examples_temperature(v: &mut Vec<::rustling::train::Example<Dimension>>) {
    ...
    example!(v, check_temperature(32.0, Some("celsius")), "trenta due gradi celsius", "32°C", "32° C", "32° c", "32°c", "32 °c");
...
pub fn examples_finance(v: &mut Vec<::rustling::train::Example<Dimension>>) {
    ...
    // todo_example!(v, check_finance(3000000.0, Some("EUR"), Precision::Exact), "tre milioni di euro");
}